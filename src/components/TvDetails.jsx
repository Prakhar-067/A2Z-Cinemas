...
const Tvdetails = () => {
    ...
    useEffect(() => {dispatch(asyncloadtv(id))
        return () =>{
            dispatch(removetv());
        }
    },[id])
  ...
  <div className="flex gap-x-5 items-center text-white">
    ...
    {info.watchproviders.buy.map((w) => (
     ...
     {/* <HorizontalCards data={info.detail.seasons.length>0 ? info.detail.seasons}/> */}
      <div className='w-full h-[38vh]  flex overflow-x-auto overflow-y-hidden p-2'>
      {info.detail.seasons.length >0 ? info.detail.seasons.map((s,i)=>(
        <div key={i} className='w-[15vh] mr-[10%] '>
           <img className=" shadow-[8px_17px_38px_2px_rgba(0,0,0,.5)] h-[30vh] min-w-[14vw]  object-cover"src={`https://image.tmdb.org/t/p/original/${s.poster_path}`} alt="" />
             <h1 className='text-2xl text-zinc-400 mt-3 font-semibold '>
             {s.original_title|| s.name || s.title ||s.original_name }

             </h1>
        </div>
      )):(<p className=" text-3xl mt-5 text-center text-white">Nothing to Show</p>)}
